...
function SfxCard({ sfx, onRefresh }: { sfx: SfxEntry; onRefresh: () => void }) {
  ...
  const handleReplace = async () => {
    setReplacing(true);
    try {
      const res = await fetch(`${API}/sfx/${sfx.id}/replace`, { method: 'POST' });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="text-xs text-text-muted">{sfx.id}</p>
              {!sfx.active && <span className="text-[10px] text-red-400 bg-red-900/20 px-1.5 rounded">inaktiv</span>}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <label className="flex items-center gap-1 mr-1 cursor-pointer" title={sfx.active ? 'Deaktivieren' : 'Aktivieren'}>
            <input
              type="checkbox"
              checked={sfx.active}
          ...
          <button
            ...
            disabled={replacing || uploading || deleting}
            className="p-1.5 rounded hover:bg-surface-hover text-text-muted hover:text-brand transition-colors disabled:opacity-50"
            title="Neu generieren"
          >
            {replacing ? <Loader2 size={14} className="animate-spin" /> : <RotateCw size={14} />}
          </button>
          ...
          <button
            ...
            className="p-1.5 rounded hover:bg-red-900/30 text-text-muted hover:text-red-400 transition-colors disabled:opacity-50"
            title="Löschen"
          >
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/sfx`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: id.trim(), name: name.trim(), category }),
      });
      if (!res.ok) {
      ...
      setId(''); setName(''); setCategory('other');
      setOpen(false);
      onAdded();
    } catch (err: any) {
