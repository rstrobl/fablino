                  ...
                  return (
                    <div key={s.id} className="featured-card">
                      <div className="featured-clickable" onClick={() => playStory(s)}>
                        ...
                        <h3>{s.title}</h3>
                        {s.heroName ? <span className="story-hero">Für {s.heroName}{s.age ? `, ${Math.round(s.age)} Jahre` : ''}</span> : s.age ? <span className="story-hero">Ab {Math.round(s.age) <= 5 ? '3' : Math.round(s.age) <= 8 ? '6' : '9'} Jahre</span> : null}
